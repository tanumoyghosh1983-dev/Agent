...
// Turns a finished interview transcript (question/answer pairs) into a
// structured, detailed Markdown documentation draft — the actual payoff of
// running the interview in the first place. A human should still read and
// polish this, but it turns 20-40 minutes of loose spoken answers into an
// organized first draft instead of a wall of raw Q&A.
//
...
const SYSTEM_PROMPT = `You are a technical writer turning an interview transcript with a subject-matter expert into detailed internal documentation about a client project.
...
async function callClaude(apiKey, model, expert, project, qa) {
  const transcript = qa
...
exports.handler = async function (event) {
  ...
  try {
    const markdown = await callClaude(apiKey, model, expert, project, qa);
